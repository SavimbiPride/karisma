...
export default function TambahMentor() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage(msg === 'Email sudah digunakan' ? 'Email sudah digunakan!' : 'Gagal menambahkan mentor');
      setShowErrorNotif(true);
      setTimeout(() => setShowErrorNotif(false), 3000);
    }
  };
  ...
  return (
    <div className="min-h-screen min-w-screen bg-[#0a0a57] flex items-center justify-center relative">
      <div className="bg-white rounded-lg p-10 shadow-md w-[900px]">
        <h2 className="text-black text-2xl font-bold text-center mb-6">Tambah Mentor Baru</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-black">
          ...
          <div className="flex justify-end gap-4 mt-4">
            <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded cursor-pointer">
              Simpan
            </button>
            <button type="button" onClick={() => navigate('/list_mentor')} className="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded cursor-pointer">
              Batal
            </button>
          </div>
        </form>
